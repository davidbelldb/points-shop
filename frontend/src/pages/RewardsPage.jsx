  ...
  return (
    <div className="space-y-4 py-2">
      <div className="flex items-center justify-between">
        <Pressable as={Link} to="/account" className="text-sm font-medium text-neutral-500">Back</Pressable>
            ...
            {pending.map((r) => (
              <li key={r.id} className="flex items-center gap-3 rounded-xl border border-teal-200 bg-teal-50 p-3">
                ...
                <div className="flex shrink-0 flex-col gap-1">
                  <Pressable onClick={() => claim(r.id)} disabled={busy}
                    className="rounded-lg bg-teal-300 px-3 py-1.5 text-xs font-semibold text-teal-900 transition hover:bg-teal-400 disabled:opacity-40">
                    {r.product_id ? 'Claim' : 'Redeem'}
                  </Pressable>
                  <Pressable onClick={() => dismiss(r.id)} disabled={busy}
