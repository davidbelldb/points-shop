...
export default function NewChatPage() {
  ...
  // ── Scrolls (raven messages) feature state ──
  const scrolls = useScrolls();
  const [scrollComposeOpen, setScrollComposeOpen] = useState(false);
  const [scrollsListOpen, setScrollsListOpen] = useState(false);
  const [scrollSendFlight, setScrollSendFlight] = useState(false);
  const [scrollLandFlight, setScrollLandFlight] = useState(false);
  const [scrollPerched, setScrollPerched] = useState(false);

  // Deep-link from the arrival push (/new-chat?scrolls=1) opens the list.
  useEffect(() => {
    const p = new URLSearchParams(window.location.search);
    if (p.get('scrolls') === '1') setScrollsListOpen(true);
  }, []);

  // A freshly-arrived crow plays the landing sequence, then perches (tappable).
  useEffect(() => {
    if (scrolls.arrivedTick > 0) { setScrollLandFlight(true); setScrollPerched(false); }
  }, [scrolls.arrivedTick]);
  ...
  // URL-only feature, hard-gated to admin so it stays invisible to Katie until
  // it's merged into the live /messages page. (actual_role survives admin
  // impersonating Katie, so the receiving side can still be tested.)
  ...
  return (
    <>
      {/* ── Scrolls feature overlays ── */}
      {scrollComposeOpen && (
        <ScrollComposeModal
          settings={scrollSettings}
          onSend={scrolls.send}
          onSent={() => { setScrollComposeOpen(false); setScrollSendFlight(true); }}
          onClose={() => setScrollComposeOpen(false)}
        />
      )}
      {scrollsListOpen && (
        <ScrollsListModal
          ...
          onClose={() => { setScrollsListOpen(false); setScrollPerched(false); scrolls.refresh(); }}
        />
      )}
      {/* Fixed send branch (left) while composing */}
      {scrollComposeOpen && <ScrollBranch file={scrollSettings.send_branch_file} side="left" />}
      {/* Send flight: low-left → high-right, exits screen */}
      <CrowAnimationLayer
        frames={scrolls.config.send}
        fps={scrollFps}
        playing={scrollSendFlight}
        onComplete={() => setScrollSendFlight(false)}
      />
      {/* Landing: high-right → low-right onto the landing branch, then perch */}
      {(scrollLandFlight || scrollPerched) && (
        <>
          <ScrollBranch file={scrollSettings.land_branch_file} side="right" />
          <CrowAnimationLayer
            frames={scrolls.config.land}
            fps={scrollFps}
            playing={scrollLandFlight}
            perchOnEnd
            onComplete={() => { setScrollLandFlight(false); setScrollPerched(true); }}
            onFinalTap={() => { setScrollsListOpen(true); setScrollPerched(false); }}
          />
        </>
      )}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex shrink-0 items-center gap-3">
            <button
              type="button"
              onClick={() => setScrollsListOpen(true)}
            ...
            <div className="pt-3 pb-1">
              {recording ? (
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-1">
                  ...
                  <button
                    type="button"
                    onClick={() => { setShowMedia(false); setScrollComposeOpen(true); }}
                    title="Send a scroll"
                    className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-neutral-200 bg-white text-neutral-500 transition hover:border-amber-300 hover:text-amber-700 active:scale-95"
                  >
