...
export default function AdminEntertainmentSection({ bare = false }) {
  ...
  async function add() {
    ...
    try { await api.admin.addEntertainmentTitle(label); setDraft(''); await load(); }
  ...
  const body = (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium uppercase tracking-wide text-neutral-500">Manual titles</p>
        <p className="text-xs text-neutral-500">
          These appear on the wheel alongside any watchlist titles marked &ldquo;invite&rdquo;. A &ldquo;Bum Show&rdquo; segment is always added automatically.
        </p>
        <div className="flex gap-2">
          ...
          />
          <button onClick={add} disabled={busy || !draft.trim()} className="rounded-md bg-amber-600 px-3 py-1.5 text-sm font-semibold text-amber-900 disabled:opacity-40">Add</button>
        </div>
        {titles === null ? (
          <p className="text-xs text-neutral-400">Loading…</p>
        ) : titles.length === 0 ? (
          <p className="text-xs text-neutral-400">No manual titles yet.</p>
        ) : (
